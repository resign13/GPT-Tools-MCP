...
pub const P0_TOOLS: &[(&str, &str, &str, bool, bool, bool)] = &[
    ...
    (
        "permission_status",
        "Permission status",
        "Return the effective permission preset and actual isolation capabilities.",
        true,
        false,
        false,
    ),
    ...
    (
        "apply_patch",
        "Apply patch",
        "Apply a patch transactionally within the configured access scope. Explicit host execution also permits external paths.",
        false,
        true,
        false,
    ),
    ...
    (
        "request_permissions",
        "Request permissions",
        "Validate a concrete runtime operation under automatic approval. Valid soft permissions are granted without desktop review; workspace, network policy and isolation limits still apply.",
        true,
        false,
        false,
    ),
    (
        "get_approval_status",
        "Get approval status",
        "Read the status of an approval request for the current conversation.",
        true,
        false,
        false,
    ),
    ...
    match name {
        ...
        "exec_command" => json!({
            ...
            "properties": {
                ...
                "confirm": { "type": "boolean", "default": false },
                "filesystem_scope": { "type": "string", "enum": ["workspace", "host"], "description": "Omit to use the configured execution scope. Explicit scope must match the backend; this parameter never elevates permission." },
                "reason": { "type": "string", "default": "" }
            },
            "required": ["cmd"],
            "additionalProperties": false
...
mod tests {
    ...
    #[test]
    fn permission_and_exec_schemas_match_runtime_capabilities() {
        let permissions = input_schema("request_permissions");
        let permission_values = permissions["properties"]["permission"]["enum"]
            .as_array()
            .expect("permission enum");
        assert!(!permission_values.iter().any(|value| {
            matches!(value.as_str(), Some("long_timeout" | "write_generated_or_ignored"))
        }));
        assert_eq!(permissions["properties"]["scope"]["enum"], json!(["once"]));
        assert_eq!(permissions["properties"]["ttl_seconds"]["const"], json!(600));
        assert_eq!(
            permissions["properties"]["tool_name"]["enum"],
            json!(["exec_command"])
        );

        let exec = input_schema("exec_command");
        assert_eq!(exec["properties"]["env"]["type"], "object");
        assert_eq!(exec["properties"]["cwd"]["description"], "Alias for workdir");
    }
}
