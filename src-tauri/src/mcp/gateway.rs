...
use crate::mcp::server::{handle_request_with_identity, SharedState};
...
impl GatewayRouter {
    pub fn from_host(host_workspace_id: String) -> Result<Option<Arc<Self>>, String> {
        Self::from_host_with_approval_store(host_workspace_id, Arc::new(ApprovalStore::default()))
    }

    pub fn from_host_with_approval_store(
        host_workspace_id: String,
        approvals: Arc<ApprovalStore>,
    ...
    /// Stop sessions owned by every currently bound target before the listener
    /// generation is dropped. Each ToolContext owns its own SessionStore, so a
    /// gateway restart can revoke all target processes without touching a
    /// different gateway instance.
    pub async fn shutdown_sessions(&self, reason: &str) -> Result<(usize, usize), String> {
        let contexts = {
            let bindings = self
                .bindings
                .lock()
                .map_err(|_| "Gateway bindings are unavailable during shutdown.".to_string())?;
            let mut contexts = Vec::new();
            for binding in bindings.values() {
                if !contexts
                    .iter()
                    .any(|context: &SharedState| Arc::ptr_eq(context, &binding.context))
                {
    ...
    /// Revoke approvals for every execution root owned by this gateway.
    ///
    /// Gateway target contexts can point at a different worktree than the host
    /// profile. Clearing only the host profile path during listener shutdown
    /// would leave those target approvals usable after a restart. Snapshot the
    /// roots first so the approval store is never accessed while the bindings
    /// mutex is held.
    pub fn revoke_approvals(&self) -> usize {
        let roots = {
            ...
            bindings
                .values()
                .map(|binding| binding.context.execution_root_display())
                .collect::<std::collections::HashSet<_>>()
        };
        roots
            .into_iter()
            .map(|root| self.approvals.remove_execution_root(&root))
            .sum()
    }
    ...
    fn build_context_at_root(
        ...
        let host_policy = PolicySettings::from_runtime(&host.runtime)
            .map_err(|error| tool_err_code("invalid_permission_configuration", error, "policy"))?;
        ...
        let context = match identity {
            Some(identity) => ToolContext::try_from_workspace_with_identity(
                ...
                policy.permissions.preset.as_str().into(),
                identity,
            ),
        ...
        .map_err(tool_err)?
        .with_approval_store(self.approvals.clone());
        let context = Arc::new(context);
        crate::runtime::diagnostics::register(&target.id, "mcp", &context);
        Ok(context)
    }
...
fn profile_fingerprint(profile: &WorkspaceProfile) -> String {
    ...
    let material = json!({
        ...
        "permission_policy_version": profile.runtime.permission_policy_version,
        "isolation_policy": profile.runtime.isolation_policy,
...
pub(crate) fn intersect_policies(host: &PolicySettings, target: &PolicySettings) -> PolicySettings {
    ...
    PolicySettings {
        ...
        permissions: host.permissions.intersect(&target.permissions),
    }
}
...
pub fn handle_gateway_request_with_source(
    ...
    if !router.is_enabled() {
        return handle_request_with_identity(host, body, session_key);
    }
    let method = body.get("method").and_then(Value::as_str).unwrap_or("");
    let id = body.get("id").cloned().unwrap_or(Value::Null);
    match method {
        "initialize" => {
            let mut response = handle_request_with_identity(host, body, session_key);
        ...
        "tools/call" => {
            ...
            let mut response = handle_request_with_identity(
                &target.context,
                &request_body,
                Some(session_key),
            );
        ...
        _ => handle_request_with_identity(host, body, session_key),
    }
}
...
mod tests {
    ...
    #[test]
    fn host_access_gateway_intersection_and_pinned_conversations() {
        use crate::tools::policy::IsolationPolicy;
        let (_root, main, worktree) = repository();
        let mut host = profile(&main, "host", true, vec!["host".into(), "target".into()]);
        host.runtime.permission_mode = "full_access".into();
        host.runtime.permission_policy_version = 1;
        host.runtime.isolation_policy = IsolationPolicy::Host;
        let mut target = profile(&worktree, "target", false, vec![]);
        let restricted = GatewayRouter::from_profiles(host.clone(), vec![target.clone()]);
        assert_eq!(restricted.select_workspace("restricted", "target")["ok"], true);
        assert!(!restricted.context_for("restricted", "get_default_cwd").unwrap().host_access());
        target.runtime = host.runtime.clone();
        let router = GatewayRouter::from_profiles(host, vec![target]);
        assert_eq!(router.select_workspace("a", "host")["ok"], true);
        assert_eq!(router.select_workspace("b", "target")["ok"], true);
        let a = router.context_for("a", "get_default_cwd").unwrap();
        let b = router.context_for("b", "get_default_cwd").unwrap();
        assert!(a.host_access() && b.host_access());
        assert_ne!(a.default_cwd_path(), b.default_cwd_path());
        assert!(!Arc::ptr_eq(&a.sessions, &b.sessions));
        assert_eq!(router.pin_workspace_context("b", &json!({"path": worktree}))["ok"], true);
        git(&worktree, &["checkout", "--detach"]);
        assert!(router.context_for("b", "get_default_cwd").is_ok());
        assert_eq!(router.get_workspace_context("b")["ok"], true);
    }
    ...
    #[test]
    fn gateway_shutdown_revokes_approvals_for_target_execution_roots() {
        let host_dir = tempfile::tempdir().expect("host");
        let target_dir = tempfile::tempdir().expect("target");
        let host = profile(
            host_dir.path(),
        ...
        let scope = crate::security::approval::ApprovalScope {
            context: "session".into(),
            task: String::new(),
            execution_root: context.execution_root_display(),
            policy_revision: context.permission_revision(),
        };
        let pending = router
            .approvals
            .request(scope, "exec_command", &json!({"cmd": "custom-tool"}))
            .expect("approval");

        assert_eq!(router.revoke_approvals(), 1);
        assert!(router.approvals.status("session", &pending.approval_id).is_none());
    }
