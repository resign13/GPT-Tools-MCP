...
struct AppState {
    ...
    /// Server-owned approval origin. Actions does not receive a ChatGPT
    /// conversation identity, so all retries for this listener share this
    /// generation-scoped value and cannot consume MCP approvals.
...
async fn serve(
    ...
    crate::runtime::diagnostics::register(profile_id, "actions", &ctx);
    ...
    let state = AppState {
        ...
        approval_origin: format!("actions:{profile_id}:{}", Uuid::new_v4()),
    };
    ...
    let serve_result = axum::serve(listener, app)
        ...
        .await;
    match sessions.shutdown_all("server_restart").await {
...
async fn execute_action(
    ...
    let structured = if tools::registry::MUTATING_TOOLS.contains(&tool_name.as_str()) {
        ...
        tools::call_tool_with_identity(
            state.ctx.as_ref(),
            &tool_name,
            &arguments,
            Some(&state.approval_origin),
        )
    } else {
        tools::call_tool_with_identity(
            state.ctx.as_ref(),
