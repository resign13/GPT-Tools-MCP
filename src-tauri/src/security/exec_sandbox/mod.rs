...
#[cfg(windows)]
pub(crate) use windows::{spawn_host, process_elevated};
...
impl SandboxStatus {
    pub fn boundary(&self) -> &'static str {
        if self.mode == ExecutionIsolationMode::Host {
            "host"
...
impl ExecSandboxManager {
    ...
    pub fn for_policy(context: &ExecutionContext, policy: crate::security::permission::IsolationPolicy) -> Self {
        if policy == crate::security::permission::IsolationPolicy::Host {
            Self { mode: ExecutionIsolationMode::Host }
        } else { Self::for_execution_context(context) }
    }
    ...
    pub fn status(&self, execution_root: &Path) -> SandboxStatus {
        if self.mode == ExecutionIsolationMode::Host {
            return SandboxStatus { available: cfg!(windows), enforced: false, implementation: "host_process".into(), execution_root: execution_root.to_path_buf(), fallback_allowed: false, mode: self.mode };
        }
        // Report the selected runner, not the availability of an unused backend.
        // Legacy contexts use the direct runner even on AppContainer-capable hosts.
        if self.mode == ExecutionIsolationMode::LegacyPolicyOnly {
            return SandboxStatus {
                available: true,
                enforced: false,
                implementation: "policy_only".into(),
    ...
    pub fn require_enforced(&self, execution_root: &Path) -> WorkspaceResult<()> {
        let status = self.status(execution_root);
        if self.mode == ExecutionIsolationMode::Host && !status.available {
            return Err(WorkspaceError::Tool { code: "HOST_BACKEND_UNSUPPORTED", message: "Host backend is implemented for Windows only.".into(), category: "runtime", retryable: false });
        }
...
mod tests {
    ...
    #[test]
    fn permission_policy_only_status_does_not_claim_appcontainer() {
        let root = tempfile::tempdir().expect("execution root");
        let manager = ExecSandboxManager {
            mode: ExecutionIsolationMode::LegacyPolicyOnly,
        };
        let status = manager.status(root.path());
        assert!(status.available);
        assert!(!status.enforced);
        assert_eq!(status.implementation, "policy_only");
        assert_eq!(status.boundary(), "policy_only");
        assert_eq!(status.execution_root, root.path());
        assert!(manager.require_enforced(root.path()).is_ok());
    }
}
