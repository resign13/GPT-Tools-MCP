...
impl WorkspaceContextPin {
    pub fn create(
        ...
        Self::create_for_access(configured_root, options, source, false)
    }

    pub(crate) fn create_for_access(configured_root: &Path, options: &PinOptions, source: &str, host_access: bool) -> Result<Self, ContextError> {
        ...
        let identity = if host_access {
            crate::tools::execution_context::probe_git_identity_with_detached(&active_root, deadline, true).map_err(|error| ContextError::new(error.code, error.message))?
        } else { probe_identity(&active_root, deadline)? };
        ...
        if !host_access && is_protected_branch(&identity.branch)
    ...
    pub fn validate(&mut self) -> Result<GitIdentity, ContextError> {
        ...
        let deadline = Instant::now() + GIT_TIMEOUT;
        let identity = crate::tools::execution_context::probe_git_identity_with_detached(&active_root, deadline, self.host_access)
        ...
        if path_key(&identity.root) != path_key(&self.active_root)
            ...
            || (!self.host_access && identity.branch != self.branch)
        {
            return Err(ContextError::new(
                "WORKSPACE_CONTEXT_MISMATCH",
        ...
        if self.host_access {
            self.branch = identity.branch.clone();
