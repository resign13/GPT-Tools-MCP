...
impl Harness {
    ...
    /// A completed operation in this host session may intentionally move HEAD.
    /// Only its expected Git state is refreshed; external changes before the next
    /// operation still go through check_baseline.
    pub(crate) fn refresh_host_expected_state(&self, task_id: &str) -> HarnessResult<TaskSession> {
        let mut task = self.task(task_id)?;
        let observed = capture_baseline(&self.workspace_root);
        task.baseline.branch = observed.branch;
        task.baseline.head = observed.head;
        task.expected_fingerprint = observed.worktree_fingerprint;
        task.updated_at = timestamp();
        self.store.save_task(&task)?;
        Ok(task)
    }
