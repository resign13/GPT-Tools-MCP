...
pub(crate) fn spawn(
    ...
    spawn_with_backend(policy, program, args, env, false)
}

pub(crate) fn spawn_host(policy: &SandboxPolicy, program: &str, args: &[String], env: &[(String, String)]) -> io::Result<SandboxChild> {
    spawn_with_backend(policy, program, args, env, true)
}

fn spawn_with_backend(policy: &SandboxPolicy, program: &str, args: &[String], env: &[(String, String)], host: bool) -> io::Result<SandboxChild> {
    let (grants, capabilities) = if host {
        (None, None)
    } else {
        std::fs::create_dir_all(&policy.temp_root)?;
        validate_policy(policy)?;
        let profile = AppContainerProfile::create(policy)?;
        (Some(AclGrants::grant(policy, &profile.sid)?), Some(profile.security_capabilities(policy.network_allowed)?))
    };

    let pipes = PipeSet::create()?;
    let child_pipes = ChildPipeHandles([pipes.child_stdin, pipes.child_stdout, pipes.child_stderr]);
    let mut command_line = if host && args.len() == 4 && args[..3] == ["/d", "/s", "/c"] {
        // cmd.exe parses its /c tail itself, not with CommandLineToArgvW.
        wide(&format!("{} /d /s /c \"{}\"", windows_quote(program), args[3]))
    } else { command_line(program, args) };
    ...
    let mut flags = EXTENDED_STARTUPINFO_PRESENT | CREATE_SUSPENDED | windows::Win32::System::Threading::CREATE_NO_WINDOW;
...
struct ChildPipeHandles([HANDLE; 3]);
impl Drop for ChildPipeHandles {
    fn drop(&mut self) { for handle in self.0 { close_handle(handle); } }
}
...
struct AttributeList {
    ...
    _capabilities: Option<SecurityCapabilities>,
    capabilities: Option<Box<SECURITY_CAPABILITIES>>,
    _handle_list: Vec<HANDLE>,
}

impl AttributeList {
    fn new(mut capabilities: Option<SecurityCapabilities>, handle_list: Vec<HANDLE>) -> io::Result<Self> {
        let mut size = 0usize;
        let attribute_count = u32::from(capabilities.is_some()) + u32::from(!handle_list.is_empty());
        let _ =
        ...
        unsafe { InitializeProcThreadAttributeList(Some(list), attribute_count, None, &mut size) }
            .map_err(win32_io_error)?;
        let raw_capabilities = capabilities.as_mut().map(|value| Box::new(value.as_raw()));
        ...
        if let Some(capabilities) = attribute_list.capabilities.as_ref() {
        unsafe {
            UpdateProcThreadAttribute(
                list,
                0,
                PROC_THREAD_ATTRIBUTE_SECURITY_CAPABILITIES as usize,
                Some(capabilities.as_ref() as *const _ as *const _),
...
/// Query the actual primary token; group membership alone is insufficient under UAC.
pub(crate) fn process_elevated() -> Option<bool> {
    use windows::Win32::Security::{GetTokenInformation, TokenElevation, TOKEN_ELEVATION, TOKEN_QUERY};
    use windows::Win32::System::Threading::{GetCurrentProcess, OpenProcessToken};
    let mut raw = HANDLE::default();
    unsafe { OpenProcessToken(GetCurrentProcess(), TOKEN_QUERY, &mut raw) }.ok()?;
    let token = unsafe { OwnedHandle::from_raw_handle(raw.0 as *mut _) };
    let mut elevation = TOKEN_ELEVATION::default();
    let mut returned = 0;
    unsafe { GetTokenInformation(HANDLE(token.as_raw_handle() as *mut _), TokenElevation, Some(&mut elevation as *mut _ as *mut c_void), size_of::<TOKEN_ELEVATION>() as u32, &mut returned) }.ok()?;
