...
impl ToolContext {
    /// Replace the default test-local approval store with the application-owned
    /// service. Context construction remains backwards compatible for callers
    /// that do not participate in the desktop approval lifecycle.
    ...
    pub fn try_from_workspace_with_harness_root_and_identity(
        ...
        if policy.permissions.isolation == crate::security::permission::IsolationPolicy::Host && !policy.permissions.host_access() {
            return Err(WorkspaceError::invalid_argument("Host execution requires an explicitly saved full_access policy."));
        }
        if policy.permissions.isolation == crate::security::permission::IsolationPolicy::Compatibility {
            return Err(WorkspaceError::Tool {
                code: "SANDBOX_BACKEND_NOT_VALIDATED",
                message: "Compatibility isolation is experimental and has not passed the desktop toolchain gate.".into(),
                category: "security",
                retryable: false,
            });
        }
        let mut execution = identity
            .map(|identity| {
        ...
        if policy.permissions.host_access() { execution.enable_host_access(); }
        let root = execution.execution_root().to_path_buf();
        let exec_sandbox = ExecSandboxManager::for_policy(&execution, policy.permissions.isolation);
    ...
    pub fn host_access(&self) -> bool { self.policy.permissions.host_access() }

    pub fn permission_mode(&self) -> &'static str {
        self.policy.permission_snapshot().preset.as_str()
    }

    pub fn permission_revision(&self) -> String {
        let policy = serde_json::to_string(self.policy.permission_snapshot())
            .unwrap_or_else(|_| "invalid-policy".into());
        format!("{}:{policy}", self.execution_fingerprint())
    }

    pub(crate) fn approval_store(&self) -> &ApprovalStore {
        &self.approvals
    }
    ...
    pub(crate) fn execution_snapshot(&self) -> ExecutionContext {
        self.execution.lock().expect("execution lock").clone()
    }
    ...
    fn resolve_path_from_base(
        ...
        if !matches!(intent, PathIntent::Read) && !(self.host_access() && matches!(intent, PathIntent::Write | PathIntent::CommandCwd)) {
        ...
        if !self.host_access() && matches!(intent, PathIntent::Write) && candidate.is_symlink() {
