...
impl GatewayRouter {
    pub(super) fn pin_workspace_context(&self, session_key: &str, args: &Value) -> Value {
        ...
        let create = if snapshot.context.host_access() {
            WorkspaceContextPin::create_for_access(PathBuf::from(&target.path).as_path(), &options, snapshot.session_key_source.as_str(), true)
        } else {
            WorkspaceContextPin::create(PathBuf::from(&target.path).as_path(), &options, snapshot.session_key_source.as_str())
        };
        let pin = match create {
            Ok(pin) => pin,
            Err(error) => return error.tool_value(None),
        };
