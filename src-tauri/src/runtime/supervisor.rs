...
impl Default for RuntimeSupervisor {
    fn default() -> Self {
        Self {
            entries: HashMap::new(),
            approvals: std::sync::Arc::new(ApprovalStore::default()),
        }
    }
}

impl RuntimeSupervisor {
    pub fn with_approval_store(approvals: std::sync::Arc<ApprovalStore>) -> Self {
        Self {
            entries: HashMap::new(),
            approvals,
        }
    ...
    pub fn drop_workspace(&mut self, profile: &WorkspaceProfile) {
        ...
        // A deleted workspace must not leave approved operations usable through
        // a stale MCP/Actions context. Keep approvals for other execution roots.
        self.approvals.remove_execution_root(&profile.path);
    }
