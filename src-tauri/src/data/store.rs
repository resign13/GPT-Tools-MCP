...
impl DataStore {
    ...
    pub fn remove(&mut self, id: &str) -> AppResult<Option<WorkspaceProfile>> {
        if !self.data.profiles.iter().any(|profile| profile.id == id) {
            return Ok(None);
        }
        self.update_data(|data| Ok(remove_workspace_data(data, id)))
    }
    ...
    pub fn init_shared_secrets(&mut self) -> AppResult<()> {
        if SHARED_KEYS
            .iter()
            .all(|key| self.data.shared_secrets.contains_key(*key))
        {
            return Ok(());
        }
        self.update_data(|data| {
