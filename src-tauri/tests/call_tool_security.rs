...
fn safe_permission_mode_blocks_network_looking_command() {
    let policy = coding_tools_mcp_desktop_lib::tools::policy::PolicySettings {
        permissions: coding_tools_mcp_desktop_lib::tools::policy::ExecutionPolicy::from_config(
            "safe", 0, Default::default(),
        ).unwrap(),
        ..Default::default()
    };
    ...
    assert!(err.0.starts_with("NETWORK_NOT_ALLOWED:"), "{}", err.0);
}
