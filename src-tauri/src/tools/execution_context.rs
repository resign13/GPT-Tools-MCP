...
impl ExecutionContext {
    ...
    pub fn validate_resolved_path(
        ...
        if !inside_execution {
            ...
            if let Ok(identity) = probe_git_identity(&anchor, Instant::now() + GIT_TIMEOUT) {
        ...
        let identity = match probe_git_identity(&anchor, Instant::now() + GIT_TIMEOUT) {
    ...
    pub fn validate(&mut self) -> WorkspaceResult<()> {
        ...
        if path_key(&current_cwd) != path_key(&self.execution_root) {
            match probe_git_identity(&current_cwd, Instant::now() + GIT_TIMEOUT) {
        ...
        let actual = probe_git_identity(&active_root, deadline).map_err(|error| {
            context_mismatch(
                error.message,
                json!({
                    "expected": identity_summary(&expected),
